...
export function InfinityJourney() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const travelled = Math.min(scrollableDistance, Math.max(0, -rect.top));
      const progress = Math.min(1, Math.max(0.025, travelled / scrollableDistance));
      ...
      >
        <defs>
          <linearGradient id="journey-gold" x1="220" y1="90" x2="1010" y2="4560" gradientUnits="userSpaceOnUse">
            ...
            <stop offset="0.52" stopColor="var(--color-warm-ivory)" stopOpacity="0.58" />
          ...
          <linearGradient id="journey-glow" x1="180" y1="70" x2="980" y2="4600" gradientUnits="userSpaceOnUse">
          ...
          <linearGradient id="journey-highlight" x1="300" y1="120" x2="900" y2="4540" gradientUnits="userSpaceOnUse">
          ...
          <filter id="journey-light-blur" x="-25%" y="-4%" width="150%" height="108%" colorInterpolationFilters="sRGB">
            <feGaussianBlur stdDeviation="5" />
          </filter>
        </defs>
        <path className="journey-path-base-glow" d={JOURNEY_PATH} fill="none" stroke="url(#journey-glow)" strokeLinecap="round" strokeWidth="22" filter="url(#journey-light-blur)" vectorEffect="non-scaling-stroke" />
        <path className="journey-path-base" d={JOURNEY_PATH} fill="none" stroke="url(#journey-gold)" strokeLinecap="round" strokeWidth="7" vectorEffect="non-scaling-stroke" />
        <path className="journey-path-progress journey-path-progress-glow" pathLength="1" d={JOURNEY_PATH} fill="none" stroke="url(#journey-glow)" strokeLinecap="round" strokeWidth="14" filter="url(#journey-light-blur)" vectorEffect="non-scaling-stroke" />
        <path className="journey-path-progress journey-path-progress-main" pathLength="1" d={JOURNEY_PATH} fill="none" stroke="url(#journey-gold)" strokeLinecap="round" strokeWidth="3.8" vectorEffect="non-scaling-stroke" />
        <path className="journey-path-progress-segment journey-path-progress-highlight" pathLength="1" d={JOURNEY_PATH} fill="none" stroke="url(#journey-highlight)" strokeDasharray="0.04 0.17 0.08 0.24 0.045 0.425" strokeLinecap="round" strokeWidth="1.45" vectorEffect="non-scaling-stroke" />
        {JOURNEY_SPARKS.map((spark) => (
          <circle key={`${spark.cx}-${spark.cy}`} className="journey-spark" cx={spark.cx} cy={spark.cy} r={spark.r} fill="url(#journey-spark)" />
        ))}
      </svg>
    </div>
  );
