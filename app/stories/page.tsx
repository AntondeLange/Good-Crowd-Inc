        ...
        <div className="container-crowd">
          <div className="grid gap-8 lg:grid-cols-[0.82fr_1.18fr] lg:items-end">
            <div>
              ...
              <h2 className="display mt-4 text-4xl md:text-6xl">Regional stories worth sharing.</h2>
            </div>
            <div className="flex flex-wrap gap-3 lg:justify-end">
              {storyCategories.map((category) => (
                <span key={category} className="rounded-full border border-crowd-brown/15 bg-crowd-mist px-4 py-2 text-sm font-bold text-crowd-brown/75">
                  {category}
                </span>
              ))}
            </div>
          </div>
          <div className="mt-12"><ArticleGrid articles={approvedGoodReads} /></div>
        </div>
      </section>
      ...
      <section className="section-pad bg-crowd-brown text-crowd-cream">
        <div className="container-crowd grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          <div>
            ...
            <h2 className="display mt-4 text-4xl md:text-6xl">Real people. Real places. Real stories.</h2>
          </div>
          <p className="max-w-2xl text-lg leading-8 text-crowd-cream/75 lg:justify-self-end">Good Reads will grow with stories from communities, events and regional Australia as there is more to share.</p>
        </div>
      </section>

      <PageCta
        eyebrow="Work with us"
        title="Have a regional story worth creating together?"
        body="Good Crowd works with councils, tourism bodies, sponsors, community organisations and event partners to create stronger regional connections and experiences."
        href="/work-with-us"
